...
organizationsRouter.post('/', async (req: Request, res: Response) => {
  try {
    const { name, address, zone, logo, logo_color, phone, email, has_tablet, participates_in_offers } = req.body
    const { rows } = await pool.query(
      `INSERT INTO organizations (name, address, zone, logo, logo_color, phone, email, has_tablet, participates_in_offers)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *`,
      [name, address, zone, logo ?? '', logo_color ?? '#A855F7', phone ?? '', email ?? '', has_tablet ?? false, participates_in_offers ?? false],
    )
