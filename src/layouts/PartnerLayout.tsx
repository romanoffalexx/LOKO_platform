      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="sticky top-0 z-20 flex items-center gap-2 md:gap-4 border-b border-loko-bg-border bg-loko-bg-base/85 px-4 md:px-6 py-3 md:py-4 backdrop-blur-xl">
          ...
          <div className="flex items-center gap-2 sm:hidden">
            <IconLogo size={22} />
            <span className="text-sm font-semibold text-loko-text-primary">Партнёр</span>
          </div>
          <div className="hidden sm:block text-sm text-loko-text-secondary truncate">Партнёр · {user?.organization_name || 'Кабинет'}</div>
        ...
        {mobileMoreOpen && (
          <div className="absolute inset-x-0 bottom-14 border-t border-loko-bg-border bg-loko-bg-base/98 backdrop-blur-xl px-4 py-3 shadow-2xl">
            <div className="grid grid-cols-2 gap-1">
              <NavLink to="/partner/notifications" onClick={() => setMobileMoreOpen(false)} className={({ isActive }) => `flex items-center gap-2 rounded-xl px-3 py-2.5 text-sm transition-colors ${isActive ? 'bg-loko-pink/10 text-loko-pink font-medium' : 'text-loko-text-secondary hover:bg-loko-bg-elevated/60'}`}>
                <IconBell size={18} /><span>Уведомления</span>
              </NavLink>
              <NavLink to="/partner/settings" onClick={() => setMobileMoreOpen(false)} className={({ isActive }) => `flex items-center gap-2 rounded-xl px-3 py-2.5 text-sm transition-colors ${isActive ? 'bg-loko-pink/10 text-loko-pink font-medium' : 'text-loko-text-secondary hover:bg-loko-bg-elevated/60'}`}>
                <IconSettings size={18} /><span>Настройки</span>
              </NavLink>
              <button onClick={() => { setMobileMoreOpen(false); handleLogout() }} className="flex items-center gap-2 rounded-xl px-3 py-2.5 text-sm text-loko-text-secondary hover:bg-loko-bg-elevated/60 transition-colors">
                <IconLogout size={18} /><span>Выйти</span>
              </button>
            </div>
          </div>
        )}
